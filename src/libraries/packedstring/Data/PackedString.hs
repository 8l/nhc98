...
filterPS predicate ps = packString (filter predicate (unpackPS ps))
...
takeWhilePS :: (Char -> Bool) -> PackedString -> PackedString
takeWhilePS predicate ps = packString (takeWhile predicate (unpackPS ps))

-- | The 'dropWhilePS' function is analogous to the 'dropWhile' function.
dropWhilePS :: (Char -> Bool) -> PackedString -> PackedString
dropWhilePS predicate ps = packString (dropWhile predicate (unpackPS ps))
...
splitWithPS predicate (PS ps) =
 ...
 where
  ...
  splitify n 
   ...
   | otherwise =
      let
       break_pt = first_pos_that_satisfies predicate ps len n
      in
...
first_pos_that_satisfies :: (Char -> Bool) -> UArray Int Char -> Int -> Int
                         -> Int
first_pos_that_satisfies predicate ps len n = 
   case [ m | m <- [n..len-1], predicate (ps ! m) ] of
	[]    -> len
	(m:_) -> m
...
hGetPS h i = do
  arr <- newArray_ (0, i-1)
  l <- hGetArray h arr i
  chars <- mapM (\j -> readArray arr j >>= return.chr.fromIntegral) [0..l-1]
...
#ifdef __GLASGOW_HASKELL__
-- Folding and unfolding is handled by pretending PackedString is a newtype on String
instance Data PackedString where
     gfoldl k z x = z packString `k` (unpackPS x)
     gunfold k z _ = k $ z packString
